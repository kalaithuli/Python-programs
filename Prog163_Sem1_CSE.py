...
def generateNewPuzzle(numSlides):
    ...
    for i in range(numSlides):
        ...
        lastMove = move
        counts=counts+1
        #pygame.display.counts()
        print(counts)
        mycon = sql.connect(host="localhost", user="root", passwd="thuli@4132", database="Highscore")
        if mycon.is_connected == False:
            print("Error connecting ")
        cursor = mycon.cursor()
        insert_statement = ("INSERT INTO Highscore (Score)"
                            "VALUES(%s)")
        cursor.execute(insert_statement, counts)
        mycon.commit()
